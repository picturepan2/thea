...
<head>
  ...
  <meta http-equiv="x-ua-compatible" content="ie=edge">
	<title><?php if(is_single() || is_page() || is_archive() || is_404() || is_search()) { ?><?php wp_title('|',true,'right'); ?><?php } ?><?php bloginfo( 'name' ); ?><?php if( $paged == "" ) $pagenum = ""; else echo $pagenum = " - Page ".$paged ; ?></title>
        ...
        <div class="navbar-section">
          <div id="search" class="search<?php echo get_search_query() ?  ' active' : ''; ?>">
            <form method="get" class="search-form" name="searchform" action="<?php echo esc_url( home_url( '/' ) ); ?>">
              <input type="text" class="search-input form-input" value="<?php the_search_query(); ?>" placeholder="Search" name="s">
